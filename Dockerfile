...
RUN sed -i.bak 's/drivername="postgresql"/drivername="cockroachdb+psycopg2"/' /app/bazarr/bin/bazarr/app/database.py && \
  ...
  sed -i.bak '57s/poolclass=NullPool\,\ isolation_level=\"AUTOCOMMIT\"/poolclass=QueuePool\,\ pool_size=20\,\ isolation_level=\"AUTOCOMMIT\",\ pool_pre_ping=True/' /app/bazarr/bin/bazarr/app/database.py && \
