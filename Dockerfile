...
RUN sed -i.bak 's/drivername="postgresql"/drivername="cockroachdb+psycopg2"/' /app/bazarr/bin/bazarr/app/database.py && \
  ...
  sed -i.bak '57s/poolclass=NullPool\,\ isolation_level=\"AUTOCOMMIT\"/poolclass=QueuePool\,\ pool_size=20\,\ isolation_level=\"SERIALIZABLE\",\ pool_pre_ping=True\,\ echo_pool=True\, max_overflow=10\,\ pool_recycle=3600\,\ pool_reset_on_return=\"rollback\"/' /app/bazarr/bin/bazarr/app/database.py && \
